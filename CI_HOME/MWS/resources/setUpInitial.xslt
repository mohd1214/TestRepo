...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="testMWSHost"/>
	<xsl:param name="testMWSPort"/>
	<xsl:param name="testMWSUsername"/>
	<xsl:param name="testMWSPassword"/>
	...
	<xsl:template match="DeployerSpec/Environment">
	    <Environment>
			<MWS>
				<mwsalias name="testMWS">
				<host><xsl:value-of select="$testMWSHost"/></host>
				<port><xsl:value-of select="$testMWSPort"/></port>
				<user><xsl:value-of select="$testMWSUsername"/></user>
				<pwd><xsl:value-of select="$testMWSPassword"/></pwd>
				<useSSL>false</useSSL>
