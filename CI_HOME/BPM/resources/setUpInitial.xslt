...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="testBPMHost"/>
	<xsl:param name="testBPMPort"/>
	<xsl:param name="testBPMUsername"/>
	<xsl:param name="testBPMPassword"/>
	...
	<xsl:template match="DeployerSpec/Environment">
	    <Environment>
			<ProcessModel>
				<pmalias name="testBPM">
				<host><xsl:value-of select="$testBPMHost"/></host>
				<port><xsl:value-of select="$testBPMPort"/></port>
				<user><xsl:value-of select="$testBPMUsername"/></user>
				<pwd><xsl:value-of select="$testBPMPassword"/></pwd>
				<useSSL>false</useSSL>
				<Test>true</Test>
				</pmalias>
				</ProcessModel>
	...
	<xsl:template match="DeployerSpec/Projects">
		<Projects>
			...
			<xsl:attribute name="description">[<xsl:value-of select="$dateTime"/>] [<xsl:value-of select="$bpmPackagesToBuild"/>]</xsl:attribute>
				...
				<MapSetMapping mapName="myDeploymentMap" setName="myDeploymentSet">	
					<alias type="ProcessModel">testBPM</alias>
				</MapSetMapping>	
